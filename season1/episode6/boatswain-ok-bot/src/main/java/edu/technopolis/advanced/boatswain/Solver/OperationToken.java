...
public class OperationToken extends Token<Character> implements CalculatingToken {
    ...
    public void calculate(ArrayDeque<Token> stack, Token currToken) throws ParseExceprion, ArithmeticException {
        ...
        switch (token) {
        case PLUS:
            stack.push(new NumberToken(a.add(b, context)));
            break;
        case MINUS:
            stack.push(new NumberToken(a.subtract(b, context)));
            break;
        case TIMES:
            stack.push(new NumberToken(a.multiply(b, context)));
            break;
        case DIVISION:
            stack.push(new NumberToken(a.divide(b, BigDecimal.ROUND_HALF_UP)));
        }
    }
